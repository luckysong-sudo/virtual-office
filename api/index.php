<?php
/**
 * Virtual Office - PHP Backend API v4
 * AI-driven virtual office with memory, relationships, daily briefing
 */
...
if ($count == 0) {
    ...
    // Seed relationships
    $rels = [
        ['bob','frank',80], ['bob','henry',65], ['carol','henry',70],
        ['alice','frank',75], ['alice','eve',60], ['grace','eve',55],
        ['david','frank',70], ['carol','alice',50], ['bob','alice',45],
    ];
    $rs = $db->prepare("INSERT INTO relationships (agent_a, agent_b, strength) VALUES (?, ?, ?)");
    foreach ($rels as $r) {
        $rs->bindValue(1,$r[0]); $rs->bindValue(2,$r[1]); $rs->bindValue(3,$r[2]);
        $rs->execute();
    }
    
    // Seed initial memories
    $memories = [
        ['alice', 'roadmap', 'Q3产品路线图正在规划中，重点关注用户增长和功能迭代', 80],
        ['bob', 'architecture', '正在重构API层，采用微服务架构提升可扩展性', 85],
        ['carol', 'design_system', '正在建立统一的设计系统，确保品牌一致性', 75],
        ['david', 'infrastructure', 'K8s集群运行稳定，CI/CD流水线已优化', 90],
        ['eve', 'test_coverage', '自动化测试覆盖率已达87%，目标是90%', 82],
        ['frank', 'team_dynamics', '团队士气良好，但需要加强跨部门协作', 70],
        ['grace', 'user_analytics', '用户活跃度环比提升15%，留存率有待改善', 78],
        ['henry', 'frontend_stack', '正在将Vue 2迁移到Vue 3，性能提升约30%', 88],
    ];
    $ms = $db->prepare("INSERT INTO agent_memory (agent_id, topic, content, confidence) VALUES (?, ?, ?, ?)");
    foreach ($memories as $m) {
        $ms->bindValue(1,$m[0]); $ms->bindValue(2,$m[1]); $ms->bindValue(3,$m[2]); $ms->bindValue(4,$m[3]);
        $ms->execute();
    }
}
...
switch ($action) {
    ...
    case 'memory':
        ...
        if ($_SERVER['REQUEST_METHOD'] === 'POST') {
            ...
            if ($agentId && $topic && $content) {
                ...
                $stmt->bindValue(4, $data['confidence'] ?? 70);
                $stmt->execute();
                $response['saved'] = true;
            }
        } else {
    ...
    case 'status':
        ...
        $response['office'] = [
            'total_agents' => $total, 'status_breakdown' => $breakdown,
            'active_tasks' => $active, 'is_night' => $isNight, 'hour' => $hour,
            'avg_energy' => $avgEnergy, 'avg_mood' => $avgMood,
        ];
        break;
    ...
    case 'task-progress':
        ...
        if ($data['task_id']) {
            ...
            $stmt->bindValue(1, $progress); $stmt->bindValue(2, $progress); $stmt->bindValue(3, $data['task_id']);
            $stmt->execute();
            if ($progress >= 100) {
    ...
    case 'chat':
        ...
        if ($agentId && $message) {
            ...
            // Enrich prompt with agent's memory
            $memories = getAgentMemories($agentId);
            $memoryContext = '';
            if ($memories) {
                $memoryContext = "\n\n你知道以下信息（来自过往对话的记忆）：\n";
                foreach ($memories as $m) {
                    $memoryContext .= "- {$m['topic']}: {$m['content']}\n";
                }
            }
            ...
            $stmt->bindValue(1, 'user'); $stmt->bindValue(2, $agentId);
            $stmt->bindValue(3, $message); $stmt->bindValue(4, $reply);
            $stmt->execute();
    ...
    case 'agent-chat':
        ...
        if ($fromId && $toId && $message) {
            ...
            $fromStmt->bindValue(1, $fromId); $fromAgent = $fromStmt->execute()->fetchArray(SQLITE3_ASSOC);
            $toStmt = $db->prepare("SELECT * FROM agents WHERE id = ?");
            $toStmt->bindValue(1, $toId); $toAgent = $toStmt->execute()->fetchArray(SQLITE3_ASSOC);
            if ($fromAgent && $toAgent) {
                ...
                $stmt->bindValue(1, $fromId); $stmt->bindValue(2, $toId); $stmt->bindValue(3, $toId); $stmt->bindValue(4, $fromId);
                $stmt->execute();
                $stmt = $db->prepare("INSERT INTO office_events (agent_id, event_type, description) VALUES (?, 'conversation', ?)");
                $stmt->bindValue(1, $fromId); $stmt->bindValue(2, "{$fromAgent['name']} 💬 {$toAgent['name']}");
                $stmt->execute();
    ...
    default:
        ...
        $response['endpoints'] = ['agents','relationships','memory','move','status','events','tasks','task-progress','boss-order','chat','agent-chat','update','personality','daily-briefing'];
        break;
}
